...
public class HttpUtils {
  ...
  private RequestConfig
      config = RequestConfig.custom().setSocketTimeout(SOCK_TIMEOUT).setConnectTimeout(CONN_TIMEOUT).build();
  ...
  public String post(String url, Map<String, ?> paramsMap, String charset, boolean isLocalToString) {
  ...
  private final String executeMethod(HttpRequestBase method, String charset, boolean isLocalToString) {
    ...
    try {
      ...
      if(200 != response.getStatusLine().getStatusCode()) {
        logger.warn("http error req={},resp={}", ToStringBuilder.reflectionToString(method), EntityUtils
      ...
      logger.error("#http client post error with reason :  msg={} # req={}", var17.getMessage(), ToStringBuilder
      ...
      try {
        ...
        logger.error("#http client close post connection error with reason : {}#", var16.getMessage());
  ...
  private final String executeMethod(HttpRequestBase method, String charset) {
    ...
    try {
      HttpResponse response = client.execute(method);
      if(200 != response.getStatusLine().getStatusCode()) {
        logger.warn("http error req={},resp={}", ToStringBuilder.reflectionToString(method), toString((HttpResponse)response, (String)charset));
        return null;
      }

      var4 = toString((HttpResponse)response, (String)charset);
    } catch (Exception var15) {
      logger.error("#http client post error with reason :  msg={} # req={}", var15.getMessage(), ToStringBuilder
          .reflectionToString(method));
      ...
      try {
        ...
        logger.error("#http client close post connection error with reason : {}#", var14.getMessage());
  ...
  private final String executeMethod(HttpRequestBase method) {
    ...
    try {
      ...
      logger.error("#http client post error with reason :  msg={} # req={}", var14.getMessage(), ToStringBuilder
          .reflectionToString(method));
      logger.error(var14.getMessage(), var14);
    } finally {
      method.releaseConnection();
      ...
      try {
        ...
        logger.error("#http client close post connection error with reason : {}#", var13.getMessage());
