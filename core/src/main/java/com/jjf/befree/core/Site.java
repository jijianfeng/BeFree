...
//    private String url; //爬取的域名
  ...
  public boolean equals(Object o) {
    ...
    if (proxyIp != site.proxyIp) {
      return false;
    }
    if (proxyPort != site.proxyPort) {
      return false;
    }
    if (isIgnoreCer != site.isIgnoreCer) {
      return false;
    }
    if (sleepTime != site.sleepTime) {
      return false;
    }
    if (encoding != site.encoding) {
      return false;
    }
    if (connectTimeout != site.connectTimeout) {
      return false;
    }
    if (socketTimeout != site.socketTimeout) {
      return false;
    }
    if (retryNumber != site.retryNumber) {
      return false;
    }
    if (connectionRequestTimeout != site.connectionRequestTimeout) {
      return false;
    }
